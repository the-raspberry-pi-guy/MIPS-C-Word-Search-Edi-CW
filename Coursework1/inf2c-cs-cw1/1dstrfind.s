...
FOUND_WORD:                             # if (contain(grid + grid_idx,word), do:
        bgt $s4, $0, NEWLINE            # if (found > 0) { // Only a print a new line if a word has been found (to stop wasted terminal line)
        j NO_NEWLINE
NEWLINE:
...
contain:
        ...
        move $t7, $s6 # temporary copy of word pointer
	move $t3, $s5 # temporary copy of string pointer
CONTAIN_LOOP:
        ...
        addi $t3, $t3, 1                # string++ increase string character pointer
